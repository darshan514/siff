...
try:
    if MODEL_PATH == "darsh90844/sif":
        tokenizer = DistilBertTokenizer.from_pretrained(MODEL_PATH, subfolder="sif_model")
        print(f"Initializing Model from {MODEL_PATH}")
        model = DistilBertForSequenceClassification.from_pretrained(MODEL_PATH, subfolder="sif_model", num_labels=2, low_cpu_mem_usage=True)
    else:
    ...
    model = BertForSequenceClassification.from_pretrained(FALLBACK_MODEL, num_labels=2, low_cpu_mem_usage=True)
